...
async fn main() -> Result<(), sqlx::Error> {
        ...
        let student_qa_macro = sqlx::query_as!(StudentQAMacro, "SELECT * FROM students WHERE StudentID =?", 5)
                .fetch_one(&pool)
                .await?;
        ...
        let student_qa_func: StudentQAFunc = sqlx::query_as("SELECT * FROM students WHERE StudentID = 5")
                .fetch_one(&pool)
                .await?;
        ...
        let tuples: Vec<(i64, String, String, String)> =
                sqlx::query_as("SELECT StudentID, FirstName, LastName, School FROM students WHERE StudentID >= ?")
                        .bind(62)
                        .fetch_all(&pool)
                        .await?;
        tuples.into_iter()
        ...
        rows.into_iter()
                .enumerate()
                .for_each(|(i, row)| println!("Enrollment pull #{}: {:?}", i, row.get::<String, &str>("Grade")));
...
WHERE e.Grade = ?
        ...
        let row: (i32,) = sqlx::query_as("SELECT COUNT(*) FROM students").fetch_one(&pool).await?;
        ...
        let row_2 = sqlx::query("SELECT * FROM enrollments WHERE EnrollmentID = ?")
                ...
                .fetch_one(&pool)
                .await?;
        let cols: Vec<_> = row_1.columns().iter().map(|col| col.name().to_string()).collect();
