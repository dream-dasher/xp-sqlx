...
SELECT
    ...
    IF(LOCATE('eu', @@hostname) > 0, 'EU', 'US') AS log_of_inferred_region,
    @@hostname AS log_of_hostname,
    @@server_uuid AS log_of_server_uuid,
    DATABASE() AS log_of_database,
    UTC_TIMESTAMP() AS log_of_query_start_time_utc
